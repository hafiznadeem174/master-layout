      ...
      <div class="card m-4 bg-white shadow rounded-lg">
        <div class="card-datatable p-4 table-responsive">
          <div class="md:flex block justify-between items-center mb-4">
            ...
            <h5 class="text-2xl text-blue-500 font-semibold">Waste Report</h5>
          ...
          <div class="overflow-x-auto border-t-[3px] border-b-2 border-blue-500">
            <table class="table-auto w-full">
              ...
              <tr>
                ...
                <th class="p-2 text-left">Ingredient Count</th>
                <th class="p-2 text-left">Responsible Person</th>
                <th class="p-2 text-left">Note</th>
                <th class="p-2 text-right">Total Loss</th>
              ...
              <tr v-for="(record, index) in filteredRecords" :key="index" class="border-t-2 border-gray-200">
                ...
                <td class="p-2">{{ record.ingredientCount }}</td>
                <td class="p-2">{{ record.responsiblePerson }}</td>
                <td class="p-2">{{ record.note }}</td>
                <td class="p-2 text-right">{{ record.totalLoss }}</td>
...
export default {
  ...
  data() {
    return {
      ...
      records: Array.from({ length: 10 }, (_, index) => ({
        ...
        totalLoss: ` ${(Math.random() * 5000).toFixed(2)} PKR`,
        ingredientCount: Math.floor(Math.random() * 10) + 1,
      ...
      let filteredData = this.records
        .filter(record => {
          ...
          const matchesSearchQuery =
            record.referenceNo.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
  ...
  methods: {
    exportToPDF() {
      ...
      const headers = ['Reference No.', 'Total Loss', 'Ingredient Count', 'Responsible Person', 'Note', 'Added By', 'Date'];
