      ...
      <div class="card m-4 bg-white shadow rounded-lg">
        <div class="card-datatable p-4 table-responsive">
          <div class="md:flex block justify-between items-center mb-4">
            <h5 class="text-2xl text-blue-500 font-semibold">Inventory Adjustments</h5>
          ...
          <div class="overflow-x-auto border-t-[3px] border-b-2 border-blue-500">
            <table class="table-auto w-full">
              ...
              <tr>
                ...
                <th class="p-2 text-left cursor-pointer" @click="sortTable('ingredientCode')">
                  Ingredient (Code)
                  <span v-if="sortColumn === 'ingredientCode'">
                    <font-awesome-icon :icon="sortOrder === 'asc' ? 'caret-up' : 'caret-down'" class="text-blue-500" />
                  </span>
                </th>

                <th class="p-2 text-left">Stock Qty/Amount</th>
                <th class="p-2 text-left">Alert Qty/Amount</th>
              ...
              <tr v-for="(record, index) in filteredRecords" :key="index" class="border-t-2 border-gray-200">
                ...
                <td class="p-2">{{ record.ingredientCode }}</td>

                <td class="p-2">{{ record.stockQtyAmount }}</td>
                <td class="p-2">{{ record.alertQtyAmount }}</td>
...
export default {
  ...
  data() {
    return {
      entries: 10, // 10 Records
      searchQuery: '',
      startDate: '',
      endDate: '',
      records: Array.from({ length: 10 }, (_, index) => ({
        ingredientCode: `Code-${index + 1}`,

        stockQtyAmount: `₨ ${(Math.random() * 5000).toFixed(2)}`,
        alertQtyAmount: `₨ ${(Math.random() * 2000).toFixed(2)}`,
        addedBy: "Jaffa'z",
        date: `01/01/202${index + 1}`,
        note: `Note for record ${index + 1}` // Add Note field
      })),
      currentPage: 1,
      sortColumn: null,
      sortOrder: 'asc',
    ...
    filteredRecords() {
      let filteredData = this.records
        .filter(record => {
          ...
          const matchesSearchQuery =
            record.ingredientCode.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
      ...
      if (this.sortColumn) {
        return filteredData.sort((a, b) => {
          ...
          if (this.sortColumn === 'ingredientCode') {
  ...
  methods: {
    exportToPDF() {
      ...
      const tableData = this.filteredRecords.map(record => [
        ...
        record.stockQtyAmount,
        record.alertQtyAmount,
        record.addedBy,
        record.date,
        record.note // Include Note in the PDF export
      ]);

      const headers = ['Ingredient (Code)', 'Stock Qty/Amount', 'Alert Qty/Amount', 'Added By', 'Date', 'Note'];
    ...
    addNewRecord() {
      // Define a new record with the "Note" field
      const newRecord = {
        ingredientCode: `Code-${this.records.length + 1}`,
        category: `Category New`,
        stockQtyAmount: `₨ ${(Math.random() * 5000).toFixed(2)}`,
        alertQtyAmount: `₨ ${(Math.random() * 2000).toFixed(2)}`,
        addedBy: "Admin",
        date: `01/01/2025`,
        note: `New note for this record` // New "Note" field
      };

      // Insert the new record at the 5th index (index 4)
      this.records.splice(4, 0, newRecord);
