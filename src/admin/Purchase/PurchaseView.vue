      ...
      <div class="card m-4 bg-white  shadow rounded-lg">
        <div class="card-datatable p-4 table-responsive">
          <div class="md:flex block justify-between items-center mb-4">
            <h5 class="text-2xl text-blue-500   font-semibold">Purchases</h5>
            ...
            <table class="table-auto w-full">
              ...
              <tr>
                ...
                <th class="p-2 text-left cursor-pointer" @click="sortTable('grandTotal')">
                  Grand Total
                  <span v-if="sortColumn === 'grandTotal'">
                      <font-awesome-icon :icon="sortOrder === 'asc' ? 'caret-up' : 'caret-down'" class="text-blue-500" />
                    </span>
                </th>
                <th class="p-2 text-left cursor-pointer" @click="sortTable('due')">
                  Due
                  <span v-if="sortColumn === 'due'">
                      <font-awesome-icon :icon="sortOrder === 'asc' ? 'caret-up' : 'caret-down'" class="text-blue-500" />
                    </span>
                </th>
              ...
              <tr v-for="(record, index) in filteredRecords" :key="index" class="border-t-2 border-gray-200">
                ...
                <td class="p-2">{{ record.grandTotal }}</td>
                <td class="p-2">{{ record.due }}</td>
                <td class="p-2 text-center">
                    <span class=" text-gray-900 px-2 py-1">
                        JAFFA'Z
                    </span>
                </td>
...
export default {
  ...
  data() {
    return {
      ...
      records: Array.from({ length: 12 }, (_, index) => ({
        ...
        grandTotal: `$${(Math.random() * 50000 + 15000).toFixed(2)}`,
        due: `$${(Math.random() * 10000).toFixed(2)}`,
    ...
    filteredRecords() {
      ...
      if (this.sortColumn) {
        return filteredData.sort((a, b) => {
          ...
          if (this.sortColumn === 'grandTotal' || this.sortColumn === 'due') {
  ...
  methods: {
    exportToPDF() {
      ...
      const headers = ['Reference No.', 'Name', 'Email', 'Grand Total', 'Due', 'Date', 'Status'];
